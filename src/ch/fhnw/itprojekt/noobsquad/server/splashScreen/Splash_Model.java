...
import ch.fhnw.itprojekt.noobsquad.server.supportClasses.Configuration;
...
public class Splash_Model extends Model {
	...
	final Task<Void> initializer = new Task<Void>() {
		...
		protected Void call() throws Exception {
			...
			serviceLocator = ServiceLocator.getServiceLocator(); // Prepares the ServiceLocator
			this.updateProgress(2, 6);
			// Initialize the resources in the service locator
			serviceLocator.setLogger(configureLogging()); // Prepares the Logger
			this.updateProgress(3, 6);
			serviceLocator.setConfiguration(new Configuration()); // Loads default properties (needed for language)
			this.updateProgress(4, 6);
			String language = serviceLocator.getConfiguration().getOption("Language"); // Will get the language
			serviceLocator.setTranslator(new Translator(language)); // The language from Configuration will be used now in Translator
			this.updateProgress(5, 6);
			// no more resources needed here
